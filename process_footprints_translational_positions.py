...
os.chdir('/home/myang/subnucleosome/unsorted_hepatocyte/motif_centric/')
...
for idx, line in hep_ref.iterrows():
    ...
    r0_frame = ref_frame[(ref_frame.length < 80) & (ref_frame.length > 10)]
    r1_frame = r1_tot[(r1_tot.length < 200) & (r1_tot.length > 10)]
    r2_frame = r2_tot[(r2_tot.length < 200) & (r2_tot.length > 10)]
    r3_frame = r3_tot[(r3_tot.length < 200) & (r3_tot.length > 10)]
    r4_frame = r4_tot[(r4_tot.length < 200) & (r4_tot.length > 10)]
    ...
    for lbl_cnt, file in enumerate(files):
        ...
        try:
            ...
            continue
        overlap_df = overlap_df[np.abs(overlap_df.midpoint - overlap_df.idx) < ((overlap_df.end - overlap_df.start) / 2)]
        overlap_df['tf'] = label

        for idx, line in overlap_df.iterrows():
            zmw = line['zmw']
            mstr = line['mstrand']
            rstr = line['rstrand']
            zmws[zmw] = (label, mstr, rstr)
        ...
        for i in tqdm(zmws):
            ...
            ref = int(i)
            nucs = overlap_df[overlap_df.zmw == ref]['idx'].values
            if len(nucs) == 0:
                continue
        ...
        if len(nuc_profiles_r0) == 0:
            continue

        np.save(f'{label_out}_{label}_motifcentric_individual_nucs_r0.npy', np.vstack(nuc_profiles_r0))
        np.save(f'{label_out}_{label}_motifcentric_individual_nucs_r1.npy', np.vstack(nuc_profiles_r1))
        np.save(f'{label_out}_{label}_motifcentric_individual_nucs_r2.npy', np.vstack(nuc_profiles_r2))
        np.save(f'{label_out}_{label}_motifcentric_individual_nucs_r3.npy', np.vstack(nuc_profiles_r3))
        np.save(f'{label_out}_{label}_motifcentric_individual_nucs_r4.npy', np.vstack(nuc_profiles_r4))

        np.save(f'{label_out}_{label}_motifcentric_acc_r0.npy', np.vstack(r0_mat))
        np.save(f'{label_out}_{label}_motifcentric_acc_r1.npy', np.vstack(r1_mat))
        np.save(f'{label_out}_{label}_motifcentric_acc_r2.npy', np.vstack(r2_mat))
        np.save(f'{label_out}_{label}_motifcentric_acc_r3.npy', np.vstack(r3_mat))
        np.save(f'{label_out}_{label}_motifcentric_acc_r4.npy', np.vstack(r4_mat))

        nuc_profiles_r0, nuc_profiles_r1, nuc_profiles_r2, nuc_profiles_r3, nuc_profiles_r4 = [], [], [], [], []

        with open(f'{label_out}_{label}_motifcentric_individual_labels.txt', 'w') as fho:
