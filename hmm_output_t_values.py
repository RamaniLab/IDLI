...
def main():
    ...
    for i in t_range:
        hmm_fho = "%s_HMMres_%s.pickle" % (fho, i)
        hmm_tot = {}
        for j in glob.glob("%s*piece*.pickle" % fhi):    
            with open(j, 'rb') as fin:
                hmmInput = pickle.load(fin)
            hmm_out = HMMv2(hmmInput, t=i / 10000)
            hmm_tot.update(hmm_out)
        with open(hmm_fho, 'wb') as fout:
            pickle.dump(hmm_tot, fout)
        foot_out = "%s_%s_footprint_out.csv" % (sys.argv[2], i)
